...
# sid.
# int opaque_init_srv(const uint8_t *pw, const ssize_t pwlen, const unsigned char *extra, const uint64_t extra_len, unsigned char rec[OPAQUE_USER_RECORD_LEN]);
...
# void opaque_session_usr_start(const uint8_t *pw, const ssize_t pwlen, unsigned char sec[OPAQUE_USER_SESSION_SECRET_LEN], unsigned char pub[OPAQUE_USER_SESSION_PUBLIC_LEN]);
def opaque_session_usr_start(pwd):
    ...
    sphinxlib.opaque_session_usr_start(pwd, len(pwd), sec, pub)
...
# void opaque_private_init_usr_start(const uint8_t *pw, const ssize_t pwlen, uint8_t *r, uint8_t *alpha);
def opaque_private_init_usr_start(pwd):
    ...
    sphinxlib.opaque_private_init_usr_start(pwd, len(pwd), r, alpha)
